"""Gemini CLI / Gemini Code Assist AI tool integration."""
...
class GeminiTool(AITool):
    """Integration for Gemini CLI and Gemini Code Assist.

    Both tools read a single GEMINI.md file at the project root.
    ...
    def is_installed(self) -> bool:
        ...
        Returns:
            True if gemini binary is found on PATH or ~/.gemini/ directory exists
        """
    ...
    def get_instructions_directory(self) -> Path:
        ...
        Raises:
            NotImplementedError: Gemini CLI only supports project-level installation
        """
        raise NotImplementedError(
            f"{self.tool_name} global installation is not supported. "
            "Gemini CLI uses project-level GEMINI.md only. "
            "Please use project-level installation instead (--scope project)."
        )

    def get_instruction_file_extension(self) -> str:
        """Get the file extension for Gemini instructions.
    ...
    def get_project_instructions_directory(self, project_root: Path) -> Path:
        """Get the directory for project-specific Gemini instructions.

        GEMINI.md lives at the project root, so the directory is the root itself.

        Args:
            project_root: Path to the project root directory

        Returns:
            Path to project root (GEMINI.md lives at root)
        """
        return project_root
    ...
    def get_instruction_path(
        ...
        """Get the path to GEMINI.md.

        Always returns project_root / GEMINI.md regardless of instruction name.
    ...
    def instruction_exists(
        ...
        """Check if an instruction section exists in GEMINI.md.
        ...
        Returns:
            True if the instruction's section markers exist in GEMINI.md
        """
        try:
    ...
    def install_instruction(
        ...
        """Install an instruction as a section in GEMINI.md.
        ...
        if path.exists():
            ...
            if start in content:
                if not overwrite:
                    raise FileExistsError(f"Instruction already exists in GEMINI.md: {instruction.name}")
    ...
    def uninstall_instruction(
        ...
        """Remove an instruction section from GEMINI.md.
