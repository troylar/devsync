"""Zed editor AI tool integration."""
...
class ZedTool(AITool):
    """Integration for Zed editor.

    Zed reads a single .rules file at the project root.
    ...
    def is_installed(self) -> bool:
        """Check if Zed is installed on the system.

        Returns:
            True if zed binary is found on PATH
        """
    ...
    def get_project_instructions_directory(self, project_root: Path) -> Path:
        """Get the directory for project-specific Zed instructions.

        .rules lives at the project root, so the directory is the root itself.

        Args:
            project_root: Path to the project root directory

        Returns:
            Path to project root (.rules lives at root)
        """
        return project_root
    ...
    def instruction_exists(
        ...
        """Check if an instruction section exists in .rules.
        ...
        Returns:
            True if the instruction's section markers exist in .rules
        """
        try:
    ...
    def install_instruction(
        ...
        """Install an instruction as a section in .rules.
        ...
        if path.exists():
            ...
            if start in content:
                if not overwrite:
                    raise FileExistsError(f"Instruction already exists in .rules: {instruction.name}")
    ...
    def uninstall_instruction(
        ...
        """Remove an instruction section from .rules.
